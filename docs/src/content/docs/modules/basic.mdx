---
title: Basic
description: This is the basic module
---

import { Card } from "@astrojs/starlight/components";

This is a basic module, it contains integer, float, string, boolean, and date types.

## Boolean

`dunna.basic.boolean()` returns a random boolean value.

```ts
// Use default probability = 0.5 (50% chance of true)
dunna.basic.boolean(); // true
dunna.basic.boolean(); // false
dunna.basic.boolean(); // false

// Modify probability (value between 0 and 1)
dunna.basic.boolean({ probability: 0.8 }); // 80% chance of true
dunna.basic.boolean({ probability: 0.2 }); // 20% chance of true
```

| Parameter   | Type   | Description               | Default |
| ----------- | ------ | ------------------------- | ------- |
| probability | number | Probability of true (0–1) | 0.5     |

## Integer

`dunna.basic.integer()` returns a random integer between 0 (not inclusive) and 10 (inclusive)

```ts
dunna.basic.integer(); // 4
dunna.basic.integer(); // 7
dunna.basic.integer(); // 4

// Customize parameters
dunna.integer({ min: 1, max: 100 });
dunna.integer({ min: -10, max: 10 });
```

| Parameter | Type   | Description                                     | Default |
| --------- | ------ | ----------------------------------------------- | ------- |
| min       | number | The minimum value of the output (inclusive)     | 0       |
| max       | number | The maximum value of the output (not inclusive) | 10      |

## Float

`dunna.basic.float()` returns a random float number between 0 (inclusive) and 10 (not inclusive)

```ts
dunna.basic.float(); // 4.364
dunna.basic.float(); // 8.834

// Parameters
dunna.float({ min: 3, max: 36, fixed: 5 });
```

| Parameter | Type   | Description                                     | Default |
| --------- | ------ | ----------------------------------------------- | ------- |
| min       | number | The minimum value of the output (inclusive)     | 0       |
| max       | number | The maximum value of the output (not inclusive) | 10      |
| fixed     | number | The number of decimal places                    | 3       |

## Letter

`dunna.basic.letter()` returns a random uppercase or lowercase letter

```ts
dunna.basic.letter(); // h
dunna.basic.letter(); // E

// Parameters
dunna.basic.letter({ casing: "upper" });
```

| Parameter | Type   | Description                                       | Default |
| --------- | ------ | ------------------------------------------------- | ------- |
| casing    | string | The casing of the output letter (upper/lower/any) | "any"   |

## Pick One

`dunna.basic.pickOne(array)` returns a random element from an array

```ts
dunna.basic.pickOne(["Alice", "Bob", "Charlie"]); // Alice
dunna.basic.pickOne(["Alice", "Bob", "Charlie"]); // Charlie
```

| Parameter | Type | Description                       | Default |
| --------- | ---- | --------------------------------- | ------- |
| array     | T[]  | The array to be randomly selected | null    |

## Pick Many

`dunna.basic.pickMany(count, array)` returns a random subset of the given array with the given count

```ts
dunna.basic.pickMany(3, [2, 1, 6, 3, 4]); // [6, 4, 1]
dunna.basic.pickMany(2, ["Alice", "Bob", "Charlie"]); // ['Bob', 'Charlie']
```

| Parameter | Type   | Description                         | Default |
| --------- | ------ | ----------------------------------- | ------- |
| count     | number | The number of elements to be picked | null    |
| array     | T[]    | The array to be randomly selected   | null    |

## Omit

`dunna.basic.omit(count, array)` returns a random subset of the given array by omitting elements based on the given count

```ts
dunna.basic.omit(2, [2, 1, 6, 3, 4]); // [1, 6 ,4]
dunna.basic.omit(2, ["Alice", "Bob", "Charlie"]); // ['Bob']
```

| Parameter | Type   | Description                          | Default |
| --------- | ------ | ------------------------------------ | ------- |
| count     | number | The number of elements to be omitted | null    |
| array     | T[]    | The array to be randomly selected    | null    |
