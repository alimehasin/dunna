...
describe('Test dunna.basic.pickMany() function', () => {
  test('check if all elements are contained from the array', () => {
    for (let i = 0; i < 100; i++) {
      const nums = [1, 2, 3, 4, 5, 6];

      const picks = dunna.basic.pickMany(3, nums);
  ...
  test('throw error when count is less than 0', () => {
    expect(() => dunna.basic.pickMany(-1, [1, 2, 3])).toThrowError();
  });

  test('throw error when count is bigger than the array length', () => {
    expect(() => dunna.basic.pickMany(10, [1, 2, 3])).toThrowError();
  });

  test('throw error when count type is float', () => {
    expect(() => dunna.basic.pickMany(5.2, [1, 2, 3])).toThrowError();
  });
});
